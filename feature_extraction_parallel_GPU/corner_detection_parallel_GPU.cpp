...
std::vector<cv::KeyPoint> CornerDetectionParallel_GPU::non_maximum_suppression_new(const std::vector<std::vector<float>> &R_values, const int& n_rows, const int& n_cols, const int& k, const int& N) {
    std::vector<cv::KeyPoint> output_corners;
    output_corners.reserve(N);
