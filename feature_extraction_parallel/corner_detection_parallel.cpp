...
std::vector<cv::Mat> CornerDetection::direction_gradients_worker(const cv::Mat& picture, const interval& interval) {
    double sumx[3] = {0};
    double sumy[3] = {0};
    // cv::Mat test_sobel = cv::Mat::zeros(n_rows, n_cols, CV_8UC1);

    const int rows_length = interval.rows.end - interval.rows.start;
    const int cols_length = interval.cols.end - interval.cols.start;


    cv::Mat Jx = cv::Mat::zeros(rows_length, cols_length, CV_64F);
    cv::Mat Jy = cv::Mat::zeros(rows_length, cols_length, CV_64F);
    cv::Mat Jxy = cv::Mat::zeros(rows_length, cols_length, CV_64F);
...
std::vector<std::vector<double>> CornerDetection::shitomasi_corner_detection_worker(const cv::Mat& Jx, const cv::Mat& Jy, const cv::Mat& Jxy, const interval& interval, const double& k){


    double jx2, jy2;
    // const double k = 0.07;

    const size_t rows_length = interval.rows.end - interval.rows.start;
    const size_t cols_length = interval.cols.end - interval.cols.start;


    std::vector<std::vector<double>> R_array(rows_length, std::vector<double>(cols_length, 0));

    for (size_t i = interval.rows.start + 2; i < interval.rows.end - 2; i++) { // if the length of the interval is 1???!?
        ...
        for (int j = interval.cols.start + 2; j < interval.cols.end - 2; j++) {
            ...
            double R = (trace / 2) - (0.5 * std::sqrt(trace * trace - 4 * det));
            // R = std::min(jx2, jy2);
